...
export const DeliveryDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      const unsubscribePickup = socketService.onOrderReadyForPickup((newPickupOrder: any) => {
        console.log('⚡ [Socket Event: ORDER_READY_PICKUP]:', newPickupOrder);
    ...
    <div className="min-h-screen bg-bg-dark text-text-primary p-4 sm:p-6 lg:p-8 space-y-6 max-w-6xl mx-auto pb-28 sm:pb-8 font-sans animate-fadeIn">
      ...
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 rounded-3xl bg-bg-darkSec border border-glass shadow-xl">
        ...
        <div className="flex items-center gap-2.5 w-full sm:w-auto justify-end">
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-black uppercase tracking-wider border flex items-center gap-2 transition-all cursor-pointer ${isOnDuty
      ...
      <div className="flex items-center gap-2 border-b border-glass pb-3">
        <button
          ...
          className={`px-5 py-2.5 rounded-xl font-extrabold text-xs uppercase tracking-wider transition-all flex items-center gap-2 cursor-pointer ${activeTab === 'ACTIVE'
        ...
        <button
          ...
          className={`px-5 py-2.5 rounded-xl font-extrabold text-xs uppercase tracking-wider transition-all flex items-center gap-2 cursor-pointer ${activeTab === 'COMPLETED'
              ? 'bg-primary text-black shadow-lg shadow-primary/20'
              : 'bg-glass text-text-muted hover:text-text-primary border border-glass'
            }`}
        >
      ...
      <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-bg-darkSec/95 border-t border-slate-200 dark:border-glass backdrop-blur-xl px-2 py-2 flex items-center justify-around shadow-2xl">
        <button
          ...
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-[10px] font-extrabold uppercase tracking-wider transition-all cursor-pointer ${activeTab === 'ACTIVE' && !isProfileModalOpen ? 'text-amber-600 dark:text-primary font-black' : 'text-slate-500 dark:text-text-muted'
            }`}
        >
          <Bike size={20} />
        ...
        <button
          type="button"
          onClick={() => { setActiveTab('ACTIVE'); setIsProfileModalOpen(false); }}
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-[10px] font-extrabold uppercase tracking-wider transition-all cursor-pointer relative ${activeTab === 'ACTIVE' && !isProfileModalOpen ? 'text-amber-600 dark:text-primary font-black' : 'text-slate-500 dark:text-text-muted'
            }`}
        >
          <MapPin size={20} />
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-[10px] font-extrabold uppercase tracking-wider transition-all cursor-pointer ${activeTab === 'COMPLETED' && !isProfileModalOpen ? 'text-amber-600 dark:text-primary font-black' : 'text-slate-500 dark:text-text-muted'
            }`}
        >
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-[10px] font-extrabold uppercase tracking-wider transition-all cursor-pointer ${isProfileModalOpen ? 'text-amber-600 dark:text-primary font-black' : 'text-slate-500 dark:text-text-muted'
            }`}
        >
          <User size={20} />
