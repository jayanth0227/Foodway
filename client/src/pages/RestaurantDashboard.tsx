...
export const RestaurantDashboard: React.FC = () => {
  ...
  const [activities, setActivities] = useState<Array<{ id: string; type: string; title: string; time: string }>>([
    { id: 'act-1', type: 'order', title: 'New order ORD-8810 received', time: '10 mins ago' },
    { id: 'act-2', type: 'menu', title: 'Menu item "Hyderabadi Biryani" enabled', time: '1 hour ago' },
    { id: 'act-3', type: 'profile', title: 'Restaurant profile opening hours updated', time: 'Yesterday' }
  ]);
  ...
  // Load Menu, Categories and Orders
  ...
  // Mock Menu Fallback
  const getInitialMenuItems = (resId: string): FoodItem[] => [
    {
      id: 'food-1',
      restaurantId: resId,
      name: 'Special Dum Biryani',
      description: 'Slow-cooked aromatic basmati rice with fragrant spices and succulent meat.',
    ...
    },
    {
      ...
      description: 'Cottage cheese cubes simmered in a rich tomato and cashew velvet gravy.',
      category: 'Main Course',
      price: 14.0,
      prepTime: '15 mins',
      isVeg: true,
    ...
    },
    {
      ...
      description: 'Leavened oven-baked flatbread brushed with garlic butter and fresh herbs.',
      category: 'Breads',
      price: 4.5,
      prepTime: '10 mins',
      isVeg: true,
    ...
    },
    {
      ...
      description: 'Warm fried dough balls soaked in cardamom syrup served with thickened sweet milk.',
      category: 'Desserts',
      price: 7.5,
      prepTime: '5 mins',
      isVeg: true,
  ...
  const getInitialOrders = (resId: string): OrderItem[] => [
    {
      id: 'ORD-8810',
      customerName: 'Siddharth Rao',
      customerPhone: '+91 98765 43210',
      customerAddress: 'Jubilee Hills, Hyderabad',
      items: '2x Special Dum Biryani, 2x Garlic Naan',
      total: 41.5,
      paymentStatus: 'paid',
      orderStatus: 'Pending',
      time: '12 mins ago',
    ...
    },
    {
      ...
      items: '1x Paneer Butter Masala, 3x Garlic Naan',
      total: 27.5,
      paymentStatus: 'paid',
    ...
    },
    {
      ...
      customerAddress: 'HITECH City, Hyderabad',
      items: '1x Special Dum Biryani, 1x Gulab Jamun',
      total: 26.0,
      paymentStatus: 'paid',
    ...
    },
    {
      ...
      items: '2x Paneer Butter Masala, 4x Garlic Naan',
      total: 46.0,
      paymentStatus: 'paid',
      orderStatus: 'Ready',
      time: '45 mins ago',
  ...
  const deleteFoodItem = async (itemId: string) => {
    ...
    if (window.confirm(`Are you sure you want to delete "${item?.name || 'this item'}"?`)) {
      ...
      logActivity('menu', `Food item "${item?.name}" was deleted.`);
    }
  };
              ...
              >
                ...
                {item.badge ? (
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${
                    isActive ? 'bg-bg-dark text-primary' : 'bg-primary/20 text-primary'
                  }`}>
                    {item.badge}
                  </span>
                ) : null}
              ...
              {navItems.map(item => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl font-bold text-xs ${
                      isActive ? 'bg-primary text-bg-dark font-black' : 'text-text-secondary bg-glass'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <Icon size={18} />
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8 animate-fadeIn w-full">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-glass pb-6">
              <div>
                <span className="text-primary font-bold text-xs uppercase tracking-widest mb-1 block">Establishment Overview</span>
                <h1 className="text-2xl sm:text-3xl font-black font-display text-primary tracking-tight">
                  Welcome back, {restaurant?.ownerName || 'Partner'} 👋
                </h1>
                <p className="text-xs text-text-muted mt-1">Here is your restaurant's live performance and order activity today.</p>
              </div>
              <div className="flex items-center gap-3">
                <button
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
              <div className="glass-panel border border-glass rounded-2xl p-4 shadow-md hover:border-primary/40 transition-all">
                <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider block">Today's Orders</span>
                <div className="text-2xl font-black font-display text-text-primary mt-2">{todayOrdersCount}</div>
                <span className="text-[10px] text-text-muted mt-1 block">Total received</span>
              </div>
              ...
              <div className="glass-panel border border-amber-500/30 bg-amber-500/5 rounded-2xl p-4 shadow-md">
                ...
                <span className="text-[10px] text-amber-500/70 mt-1 block">Awaiting response</span>
              </div>

              <div className="glass-panel border border-blue-500/30 bg-blue-500/5 rounded-2xl p-4 shadow-md">
                <span className="text-[10px] font-bold text-blue-400 uppercase tracking-wider block">Preparing</span>
                <div className="text-2xl font-black font-display text-blue-400 mt-2">{preparingCount}</div>
                <span className="text-[10px] text-blue-400/70 mt-1 block">In the kitchen</span>
              </div>

              <div className="glass-panel border border-emerald-500/30 bg-emerald-500/5 rounded-2xl p-4 shadow-md">
                <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider block">Ready</span>
                <div className="text-2xl font-black font-display text-emerald-400 mt-2">{readyCount}</div>
                <span className="text-[10px] text-emerald-400/70 mt-1 block">For pickup/rider</span>
              </div>
              ...
              <div className="glass-panel border border-primary/40 bg-primary/5 rounded-2xl p-4 shadow-md">
                <span className="text-[10px] font-bold text-primary uppercase tracking-wider block">Today's Revenue</span>
                <div className="text-2xl font-black font-display text-primary mt-2">${todayRevenue.toFixed(2)}</div>
                <span className="text-[10px] text-primary/70 mt-1 block">Gross earnings</span>
              </div>
            </div>
                          ...
                          <tr key={o.id} className="hover:bg-glass/40 transition-colors">
                            ...
                            <td className="p-4">
                              <span className={`px-2.5 py-1 rounded-md text-[10px] font-extrabold uppercase ${
                                o.orderStatus === 'Pending' ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' :
            ...
            <div className="space-y-6 animate-fadeIn w-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
                {filteredMenuItems.map(item => (
                  <div 
                    key={item.id}
                    className={`glass-panel border rounded-2xl overflow-hidden shadow-luxury flex flex-col justify-between transition-all duration-300 ${
                      !item.isAvailable ? 'opacity-65 border-glass bg-bg-dark/40' : 'border-glass hover:border-primary/40'
                    }`}
                  >
                    <div>
                      ...
                      <div className="relative h-36 w-full overflow-hidden bg-bg-dark">
                        ...
                        <div className="absolute top-2 left-2 flex gap-1.5">
                          <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider ${
                            item.isVeg ? 'bg-emerald-500/90 text-white' : 'bg-rose-600/90 text-white'
                          }`}>
                        ...
                        <div className="absolute top-2 right-2">
                          <button
                            ...
                            className={`px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-wider backdrop-blur-md transition-all ${
                              item.isAvailable ? 'bg-emerald-500/80 text-white' : 'bg-red-500/80 text-white'
                            }`}
                          >
                ...
                {['All', 'Pending', 'Accepted', 'Preparing', 'Ready', 'Completed', 'Rejected'].map(st => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-all ${
                      orderStatusFilter === st ? 'bg-primary text-bg-dark font-black' : 'bg-glass text-text-secondary hover:text-primary'
                    }`}
                  >
                    {st}
                  </button>
