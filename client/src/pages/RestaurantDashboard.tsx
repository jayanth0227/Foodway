...
export const RestaurantDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setProfileForm({
      ...
      openingTime: restProfile.openingTime || '11:00 AM',
      closingTime: restProfile.closingTime || '11:00 PM',
      image: restProfile.image || '',
      description: restProfile.description || 'Gourmet establishment serving handcrafted culinary delights.',
      cuisine: restProfile.cuisine || 'Multi-Cuisine'
    });
                  ...
                  >
                    ...
                    <div className="p-3 bg-glass-subtle/30 border-t border-glass flex items-center justify-between gap-2">
                      <button
                        type="button"
                        onClick={() => toggleFoodAvailability(item.id)}
                        className={`px-2.5 py-1.5 rounded-xl border transition-all duration-300 flex items-center gap-2 select-none group cursor-pointer ${
                          item.isAvailable
                      ...
                      >
                        ...
                        <div
                          className={`w-8 h-4 sm:w-9 sm:h-5 rounded-full p-0.5 transition-colors duration-300 flex items-center ${
                            item.isAvailable ? 'bg-emerald-500' : 'bg-slate-700'
                          }`}
                        >
                          <div
                            className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full bg-white shadow-md transform transition-transform duration-300 ${
                    ...
                    {SUGGESTED_CATEGORIES.map((cat) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-3 py-2 rounded-xl text-xs font-bold transition-all duration-200 flex items-center gap-1.5 border cursor-pointer ${
                            isSelected
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-bg-darkSec/95 border-t border-slate-200 dark:border-glass backdrop-blur-xl px-2 py-2 flex items-center justify-around shadow-2xl">
        <button
          ...
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-[10px] font-extrabold uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'dashboard' ? 'text-amber-600 dark:text-primary font-black' : 'text-slate-500 dark:text-text-muted'
          }`}
        >
        ...
        <button
          ...
          onClick={() => setActiveTab('menu')}
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-[10px] font-extrabold uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'menu' ? 'text-amber-600 dark:text-primary font-black' : 'text-slate-500 dark:text-text-muted'
          }`}
        >
          <Utensils size={20} />
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-[10px] font-extrabold uppercase tracking-wider transition-all cursor-pointer relative ${
            activeTab === 'orders' ? 'text-amber-600 dark:text-primary font-black' : 'text-slate-500 dark:text-text-muted'
          }`}
        >
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-[10px] font-extrabold uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'profile' || activeTab === 'settings' ? 'text-amber-600 dark:text-primary font-black' : 'text-slate-500 dark:text-text-muted'
          }`}
        >
          <User size={20} />
