            ...
            <div className="relative rounded-3xl overflow-hidden border border-glass shadow-luxury bg-bg-darkSec">
              <div className="h-64 sm:h-80 relative overflow-hidden">
                ...
                <div className="absolute top-6 left-6 flex gap-2">
                  <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider backdrop-blur-md shadow-lg ${
                    restaurant.isOpen ? 'bg-emerald-500/90 text-white' : 'bg-rose-600/90 text-white'
                  }`}>
              ...
              {categories.map(cat => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all cursor-pointer ${
                    selectedCategory === cat ? 'bg-primary text-black font-black shadow-md' : 'bg-glass hover:bg-glass-subtle border border-glass text-text-secondary'
                  }`}
                >
                  {cat}
                </button>
              ...
              {(['All', 'Veg', 'Non-Veg'] as const).map(dt => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    selectedDietary === dt ? 'bg-primary text-black font-extrabold' : 'text-text-muted hover:text-text-primary'
                  }`}
                >
                  {dt}
                </button>
              ...
              {filteredMenuItems.map(item => {
                ...
                return (
                  <motion.div
                    key={dishId}
                    whileHover={{ x: 4 }}
                    className={`glass-panel border rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 shadow-luxury transition-all w-full ${
                      isOutOfStock ? 'opacity-70 border-rose-500/20 bg-bg-dark/40' : 'border-glass hover:border-primary/40'
                    }`}
                  >
                    ...
                    <div className="relative w-full sm:w-36 h-36 sm:h-28 rounded-xl overflow-hidden border border-glass bg-bg-dark shrink-0">
                      ...
                      <div className="absolute top-2 left-2 flex gap-1">
                        <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase ${
                          dishObj.isVeg ? 'bg-emerald-500 text-white' : 'bg-rose-600 text-white'
                        }`}>
                          {dishObj.isVeg ? 'Veg' : 'Non-Veg'}
                        </span>
                      </div>
                      <div className="absolute top-2 right-2">
                        <span className={`px-2 py-0.5 rounded-md text-[9px] font-extrabold uppercase backdrop-blur-md ${
                          !isOutOfStock ? 'bg-emerald-500/90 text-white' : 'bg-rose-600/90 text-white'
                        }`}>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full px-3 py-2 rounded-xl text-xs font-bold text-left flex items-center justify-between transition-colors cursor-pointer ${
                        isSelected ? 'bg-primary text-black font-extrabold' : 'hover:bg-glass text-text-secondary hover:text-white'
                      }`}
                    >
                      <span>{cat}</span>
